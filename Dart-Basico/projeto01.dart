void main() {
  ...
  */
  ...
  while (j < 5) {
    /* O loop se repete enquanto jfor menor que 5.
  ...
  int contador = 0;
  while (contador < 5) {
    //o loop continua a ser executado enquanto o valor de contadorfor menor que 5.
  ...
  double exame =
      3; // exame está recendo um outro valor para verificar se caso o aluno foi aprovado no exame;
  ...
  if (mes == "Jan" || mes == "Fev" || mes == "Marc") {
    estacao =
        "Primavera"; // Se caso "mes" seja, Jan ou Fev ou Mar, o valor será "primavera".
    ...
    estacao =
        "Desconhecido"; // Caso não seja nenhum cima, a mensagem "Desconhecida" será exibida no console.
  }
  print(estacao);

  // Operadores ternários.
  List<int> numeros = [1, 2, 3, 4, 5]; // Crie uma lista de números inteiros.
  int numeroPar = numeros.firstWhere((numero) => numero % 2 == 0,
      orElse: () =>
          0); // // Encontra o primeiro número par na lista, ou retorna 0 se não houver nenhum.
  print(numeroPar > 0
      ? 'Encontrou o numero par'
      : 'Não encontrou o numero par'); // Imprima uma mensagem indicando se um número foi encontrado ou não.

  // switch case
  String semana = 'Segunda-Feira';
  switch (semana) {
    // A expressão dentro do switch é avaliada "semana".
    case 'Segunda-Feira': // O valor resultante é comparado com cada um case.
      print(
          'Primeiro dia da semana'); // Se encontrar uma correspondência, o código dentro desse case é executado.
